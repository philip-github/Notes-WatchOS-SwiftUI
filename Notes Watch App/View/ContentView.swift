        ...
        NavigationView {
            VStack {
                HStack(alignment: .center, spacing: 6) {
                    ...
                    Button {
                        // 1.only run button action when text is not empty.
                        guard text.isEmpty == false else { return }
                        
                        // 2.create a new note item and initialize it with text value.
                        let note = Note(id: UUID(), text: text)
                        
                        // 3.Add the new notes item to the notes array. (append)
                        notes.append(note)
                        
                        // 4.Make text field empty after button action.
                        text = ""
